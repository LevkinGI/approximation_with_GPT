...
2. [Глава 1. Общий алгоритм работы](#глава-1-общий-алгоритм-работы)
   1. [Подготовка окружения](#11-подготовка-окружения)
   2. [Запуск из командной строки](#12-запуск-из-командной-строки)
   3. [Сканирование структуры каталогов](#13-сканирование-структуры-каталогов)
   4. [Чтение исходных сигналов](#14-чтение-исходных-сигналов)
   5. [Загрузка теоретических оценок](#15-загрузка-теоретических-оценок)
   6. [Нормализация и препроцессинг сигналов](#16-нормализация-и-препроцессинг-сигналов)
   7. [Расчёт первичных оценок ESPRIT](#17-расчёт-первичных-оценок-esprit)
   8. [Фильтрация и подготовка кандидатов](#18-фильтрация-и-подготовка-кандидатов)
   9. [Резервный спектральный поиск](#19-резервный-спектральный-поиск)
   10. [Формирование комбинаций и границы оптимизации](#110-формирование-комбинаций-и-границы-оптимизации)
   11. [Нелинейная аппроксимация сигналов](#111-нелинейная-аппроксимация-сигналов)
   12. [Критерии успешности и выбор лучшего решения](#112-критерии-успешности-и-выбор-лучшего-решения)
   13. [Обработка случаев с единственным диапазоном](#113-обработка-случаев-с-единственным-диапазоном)
   14. [Сохранение результатов и экспорт](#114-сохранение-результатов-и-экспорт)
   15. [Генерация отчётов и визуализация](#115-генерация-отчётов-и-визуализация)
   16. [Завершение обработки и отчётность](#116-завершение-обработки-и-отчётность)
3. [Глава 2. Математические методы и их обоснование](#глава-2-математические-методы-и-их-обоснование)
   1. [Математическая модель сигнала](#21-математическая-модель-сигнала)
   2. [Дискретизация и единицы измерения](#22-дискретизация-и-единицы-измерения)
   3. [Построение корреляционных матриц для ESPRIT](#23-построение-корреляционных-матриц-для-esprit)
   4. [SVD-разложение и отбор сингулярных компонент](#24-svd-разложение-и-отбор-сингулярных-компонент)
   5. [Восстановление частот из собственных значений](#25-восстановление-частот-из-собственных-значений)
   6. [Оценивание времен затухания](#26-оценивание-времен-затухания)
   7. [Спектральные методы резервного поиска](#27-спектральные-методы-резервного-поиска)
   8. [Нелинейная минимизация невязки](#28-нелинейная-минимизация-невязки)
   9. [Оценка доверительных интервалов](#29-оценка-доверительных-интервалов)
   10. [Критерии качества и метрики стоимости](#210-критерии-качества-и-метрики-стоимости)
4. [Глава 3. Структуры данных и форматы файлов](#глава-3-структуры-данных-и-форматы-файлов)
   1. [Сырые записи (`RawRecord`)](#31-сырые-записи-rawrecord)
   2. [Набор данных (`DataSet`)](#32-набор-данных-dataset)
   3. [Результаты аппроксимации (`FittingResult`)](#33-результаты-аппроксимации-fittingresult)
   4. [Конфигурация обработки (`ProcessingConfig`)](#34-конфигурация-обработки-processingconfig)
   5. [Теоретические файлы `H_*.npy` и `T_*.npy`](#35-теоретические-файлы-h_npy-и-t_npy)
   6. [Экспортируемые таблицы](#36-экспортируемые-таблицы)
5. [Глава 4. Модули и функции](#глава-4-модули-и-функции)
   1. [Модуль `spectral_pipeline.__init__`](#41-модуль-spectral_pipeline__init__)
   2. [Модуль `spectral_pipeline.cli`](#42-модуль-spectral_pipelinecli)
   3. [Модуль `spectral_pipeline.io`](#43-модуль-spectral_pipelineio)
   4. [Модуль `spectral_pipeline.fit`](#44-модуль-spectral_pipelinefit)
   5. [Модуль `spectral_pipeline.plotting`](#45-модуль-spectral_pipelineplotting)
   6. [Утилита `find_freqs_and_visualize.py`](#46-утилита-find_freqs_and_visualizepy)
6. [Глава 5. Параметры, константы и конфигурация](#глава-5-параметры-константы-и-конфигурация)
   1. [Частотные диапазоны](#51-частотные-диапазоны)
   2. [Ограничения по времени затухания](#52-ограничения-по-времени-затухания)
   3. [Настройки ESPRIT](#53-настройки-esprit)
   4. [Параметры FFT и оконных функций](#54-параметры-fft-и-оконных-функций)
   5. [Параметры аппроксимации](#55-параметры-аппроксимации)
   6. [Настройки визуализации](#56-настройки-визуализации)
   7. [Перечень переменных среды](#57-перечень-переменных-среды)
7. [Глава 6. Обработка ошибок и диагностическое логирование](#глава-6-обработка-ошибок-и-диагностическое-логирование)
   1. [Уровни логирования](#61-уровни-логирования)
   2. [Основные сообщения и их интерпретация](#62-основные-сообщения-и-их-интерпретация)
   3. [Диагностика неуспешных аппроксимаций](#63-диагностика-неуспешных-аппроксимаций)
   4. [Отладка ESPRIT](#64-отладка-esprit)
   5. [Анализ резервного поиска](#65-анализ-резервного-поиска)
8. [Глава 7. Тестирование и верификация](#глава-7-тестирование-и-верификация)
   1. [Модульные тесты](#71-модульные-тесты)
   2. [Интеграционные проверки](#72-интеграционные-проверки)
   3. [Воспроизводимость результатов](#73-воспроизводимость-результатов)
   4. [Метрики производительности](#74-метрики-производительности)
9. [Глава 8. Руководство пользователя](#глава-8-руководство-пользователя)
   1. [Подготовка данных](#81-подготовка-данных)
   2. [Сценарии запуска](#82-сценарии-запуска)
   3. [Настройка параметров обработки](#83-настройка-параметров-обработки)
   4. [Анализ отчётов и графиков](#84-анализ-отчётов-и-графиков)
   5. [Типовые проблемы и решения](#85-типовые-проблемы-и-решения)
10. [Глава 9. Внутренние соглашения и стиль кода](#глава-9-внутренние-соглашения-и-стиль-кода)
    1. [Именование](#91-именование)
    2. [Стандарты документирования](#92-стандарты-документирования)
    3. [Обработка численных ошибок](#93-обработка-численных-ошибок)
    4. [Работа с единицами измерения](#94-работа-с-единицами-измерения)
    5. [Поддержка расширений и модификаций](#95-поддержка-расширений-и-модификаций)
11. [Глава 10. Приложения](#глава-10-приложения)
    1. [Приложение A. Подробный пример обработки одной пары](#101-приложение-a-подробный-пример-обработки-одной-пары)
    2. [Приложение B. Таблицы параметров и постоянных](#102-приложение-b-таблицы-параметров-и-постоянных)
    3. [Приложение C. Формат логов](#103-приложение-c-формат-логов)
    4. [Приложение D. Псевдокод ключевых алгоритмов](#104-приложение-d-псевдокод-ключевых-алгоритмов)
    5. [Приложение E. Список используемых сокращений](#105-приложение-e-список-используемых-сокращений)
    6. [Приложение F. План расширения функциональности](#106-приложение-f-план-расширения-функциональности)

---

<a id="введение-и-назначение"></a>
...
<a id="глава-1-общий-алгоритм-работы"></a>
...
<a id="11-подготовка-окружения"></a>
...
<a id="12-запуск-из-командной-строки"></a>
...
<a id="13-сканирование-структуры-каталогов"></a>
...
<a id="14-чтение-исходных-сигналов"></a>
...
<a id="15-загрузка-теоретических-оценок"></a>
...
<a id="16-нормализация-и-препроцессинг-сигналов"></a>
...
<a id="17-расчёт-первичных-оценок-esprit"></a>
...
<a id="18-фильтрация-и-подготовка-кандидатов"></a>
...
<a id="19-резервный-спектральный-поиск"></a>
...
<a id="110-формирование-комбинаций-и-границы-оптимизации"></a>
...
<a id="111-нелинейная-аппроксимация-сигналов"></a>
...
<a id="112-критерии-успешности-и-выбор-лучшего-решения"></a>
...
<a id="113-обработка-случаев-с-единственным-диапазоном"></a>
...
<a id="114-сохранение-результатов-и-экспорт"></a>
...
<a id="115-генерация-отчётов-и-визуализация"></a>
...
<a id="116-завершение-обработки-и-отчётность"></a>
...
<a id="глава-2-математические-методы-и-их-обоснование"></a>
...
<a id="21-математическая-модель-сигнала"></a>
...
<a id="22-дискретизация-и-единицы-измерения"></a>
...
<a id="23-построение-корреляционных-матриц-для-esprit"></a>
...
<a id="24-svd-разложение-и-отбор-сингулярных-компонент"></a>
...
<a id="25-восстановление-частот-из-собственных-значений"></a>
...
<a id="26-оценивание-времен-затухания"></a>
...
<a id="27-спектральные-методы-резервного-поиска"></a>
...
<a id="28-нелинейная-минимизация-невязки"></a>
...
<a id="29-оценка-доверительных-интервалов"></a>
...
<a id="210-критерии-качества-и-метрики-стоимости"></a>
...
<a id="глава-3-структуры-данных-и-форматы-файлов"></a>
## Глава 3. Структуры данных и форматы файлов

<a id="31-сырые-записи-rawrecord"></a>
...
<a id="32-набор-данных-dataset"></a>
...
<a id="33-результаты-аппроксимации-fittingresult"></a>
...
<a id="34-конфигурация-обработки-processingconfig"></a>
...
<a id="35-теоретические-файлы-h_npy-и-t_npy"></a>
...
<a id="36-экспортируемые-таблицы"></a>
...
<a id="глава-4-модули-и-функции"></a>
## Глава 4. Модули и функции

<a id="41-модуль-spectral_pipeline__init__"></a>
...
<a id="42-модуль-spectral_pipelinecli"></a>
...
<a id="43-модуль-spectral_pipelineio"></a>
...
<a id="44-модуль-spectral_pipelinefit"></a>
...
<a id="45-модуль-spectral_pipelineplotting"></a>
...
<a id="46-утилита-find_freqs_and_visualizepy"></a>
...
<a id="глава-5-параметры-константы-и-конфигурация"></a>
## Глава 5. Параметры, константы и конфигурация

<a id="51-частотные-диапазоны"></a>
...
<a id="52-ограничения-по-времени-затухания"></a>
...
<a id="54-параметры-fft-и-оконных-функций"></a>
...
<a id="55-параметры-аппроксимации"></a>
...
<a id="56-настройки-визуализации"></a>
...
<a id="57-перечень-переменных-среды"></a>
...
<a id="глава-6-обработка-ошибок-и-диагностическое-логирование"></a>
## Глава 6. Обработка ошибок и диагностическое логирование

<a id="61-уровни-логирования"></a>
...
<a id="62-основные-сообщения-и-их-интерпретация"></a>
...
<a id="63-диагностика-неуспешных-аппроксимаций"></a>
...
<a id="65-анализ-резервного-поиска"></a>
...
<a id="глава-7-тестирование-и-верификация"></a>
## Глава 7. Тестирование и верификация

<a id="71-модульные-тесты"></a>
### 7.1. Модульные тесты
...
<a id="72-интеграционные-проверки"></a>
...
<a id="73-воспроизводимость-результатов"></a>
...
<a id="74-метрики-производительности"></a>
...
<a id="глава-8-руководство-пользователя"></a>
## Глава 8. Руководство пользователя

<a id="81-подготовка-данных"></a>
...
<a id="83-настройка-параметров-обработки"></a>
...
<a id="84-анализ-отчётов-и-графиков"></a>
...
<a id="85-типовые-проблемы-и-решения"></a>
...
<a id="глава-9-внутренние-соглашения-и-стиль-кода"></a>
## Глава 9. Внутренние соглашения и стиль кода

<a id="91-именование"></a>
### 9.1. Именование
...
<a id="92-стандарты-документирования"></a>
...
<a id="93-обработка-численных-ошибок"></a>
...
<a id="94-работа-с-единицами-измерения"></a>
...
<a id="95-поддержка-расширений-и-модификаций"></a>
...
<a id="глава-10-приложения"></a>
## Глава 10. Приложения

<a id="101-приложение-a-подробный-пример-обработки-одной-пары"></a>
...
<a id="102-приложение-b-таблицы-параметров-и-постоянных"></a>
...
<a id="103-приложение-c-формат-логов"></a>
...
<a id="104-приложение-d-псевдокод-ключевых-алгоритмов"></a>
...
<a id="105-приложение-e-список-используемых-сокращений"></a>
...
<a id="106-приложение-f-план-расширения-функциональности"></a>
