...
def _load_guess_curves(directory: Path, field_mT: int, temp_K: int
    ...
    """Return theoretical frequency and decay curves if guess file exists."""
    ...
    hf = arr[1]
    lf = arr[2]
    hf_tau = arr[3] if arr.shape[0] > 3 else None
    lf_tau = arr[4] if arr.shape[0] > 4 else None
    return axis, hf, lf, hf_tau, lf_tau
...
def visualize_stacked(
    ...
    for ds_lf, ds_hf in triples_sorted:
        ...
        tau1, tau2 = sorted((ds_lf.fit.tau1 / NS, ds_lf.fit.tau2 / NS))
    ...
    if theory_curves is not None:
        axis, hf_th, lf_th, hf_tau_th, lf_tau_th = theory_curves
        ...
        hf_th = hf_th[mask]
        lf_th = lf_th[mask]
        if hf_tau_th is not None and lf_tau_th is not None:
            hf_tau_th = hf_tau_th[mask]
            lf_tau_th = lf_tau_th[mask]
        col_idx = 4
        fig.add_trace(
            go.Scatter(
                x=axis,
...
def visualize_without_spectra(
    ...
    fig.update_yaxes(title_text="Decay time (ns)", row=2, col=3)
