    ...
    *,
    ...
    for idx, (ds_lf, ds_hf) in enumerate(triples_sorted):
        ...
        if ds_lf.fit:
            ...
            y_lf = p.k_lf * (p.A1 * np.exp(-ds_lf.ts.t * p.zeta1) * np.sin(2*np.pi*p.f1*ds_lf.ts.t + p.phi1)) + shift
            y_hf = p.k_lf * (p.A2 * np.exp(-ds_lf.ts.t * p.zeta2) * np.sin(2*np.pi*p.f2*ds_lf.ts.t + p.phi2)) + shift
            fig.add_trace(
                go.Scattergl(
                    x=ds_lf.ts.t / NS,
                    y=y_fit,
        ...
        if ds_hf.fit:
            ...
            y_lf = p.k_hf * (p.A1 * np.exp(-ds_hf.ts.t * p.zeta1) * np.sin(2*np.pi*p.f1*ds_hf.ts.t + p.phi1)) + shift
            y_hf = p.k_hf * (p.A2 * np.exp(-ds_hf.ts.t * p.zeta2) * np.sin(2*np.pi*p.f2*ds_hf.ts.t + p.phi2)) + shift
            fig.add_trace(
                go.Scattergl(
                    x=ds_hf.ts.t / NS,
                    y=y_fit,
